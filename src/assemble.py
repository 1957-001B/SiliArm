...
class Assembler:
    ...
    def write_macho_header(self, f, code_size):
        ...
        f.write(0x00000004.to_bytes(4, byteorder='little'))  # Number of load commands (4)
        f.write(0x00000118.to_bytes(4, byteorder='little'))  # Size of load commands (280 bytes)
    ...
    def second_pass(self):
        ...
        for line in parsed:
            ...
            if opcode == OpCodes.B.value:
                ...
                if len(instruction) >= 2 and instruction[1][0] == 'REGISTER':
                    dest_reg = instruction[1][1].value if hasattr(instruction[1][1], 'value') else instruction[1][1]
                    if len(instruction) >= 3:
                        if instruction[2][0] == 'IMMEDIATE':
                            ...
                            src_reg = instruction[2][1].value if hasattr(instruction[2][1], 'value') else instruction[2][1]
                ...
                if len(instruction) >= 2 and instruction[1][0] == 'REGISTER':
                    ...
                    if len(instruction) >= 3 and instruction[2][0] == 'REGISTER':
                        src_reg = instruction[2][1].value[1] if isinstance(instruction[2][1].value, tuple) else instruction[2][1].value
                        
                        if len(instruction) > 3:
                            if instruction[3][0] == 'IMMEDIATE':
                                imm = instruction[3][1] & 0xFFF
                                machine_code = opcode | dest_reg | (src_reg << 5) | (imm << 10)
                            elif instruction[3][0] == 'REGISTER':
                                src_reg2 = instruction[3][1].value if hasattr(instruction[3][1], 'value') else instruction[3][1]
                                machine_code = opcode | dest_reg | (src_reg << 5) | (src_reg2 << 16)
                        else:
        ...
        with open(self.out, "wb") as f:
                ...
                for symbol_name, value in self.symbol_table.items():
                    f.write(str_offsets[symbol_name].to_bytes(4, byteorder='little'))  # strx
